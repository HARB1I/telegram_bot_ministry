...
#-------------------------------------------------------
    ...
    case "СПИСОК":
         if db.get_is_admin(message.chat.id) == "1":
            ...
            bot.send_message(message.chat.id, reply_markup=kb_topics, text="список мероприятий")
         else:
...
#-------------------------------------------------------
    ...
    case "МЕРОПРИЯТИЯ":     #для юзеров
        kb = db.get_topic_title()
        kb_topics = Keyboa(items=kb, copy_text_to_callback=True, front_marker="user_events|", items_in_row=3).keyboard
        bot.send_message(message.chat.id, reply_markup=kb_topics, text="выберите мероприятие на которое хотите оставить отзыв")
...
#-------------------------------------------------------
    ...
    case "МЕРOПРИЯТИЯ": #буква O английская для админа
         if db.get_is_admin(message.chat.id) == "1":
            bot.send_message(message.chat.id, reply_markup=nav.keyboard5, text="Мероприятия")
            bot.delete_message(message.chat.id, message.message_id)
         else:
             panel(message.chat.id, "Вы не являетесь админом")
...
#-------------------------------------------------------
    ...
    case "ПОЛУЧИТЬ ТАБЛИЦЫ":
        if db.get_is_admin(message.chat.id) == "1":
            ...
            columns_to_remove = ['signup', 'is_admin', 'signup_admin', 'flag_add_topic', 'is_content', 'n_content', 'id']
...
#-------------------------------------------------------
    ...
    case "УДАЛИТЬ":
        if db.get_is_admin(message.chat.id) == "1":
            kb = db.get_topic_title()
            kb_topics = Keyboa(items=kb, copy_text_to_callback=True, front_marker="admin_delete|", items_in_row=3).keyboard
            bot.send_message(message.chat.id, reply_markup=kb_topics, text="выберите мероприятие которое хотите удалить")
        else:
             panel(message.chat.id, "Вы не являетесь админом")
